...
class CelebA(Dataset):
    ...
    def __init__(self, root: str, group_ratios: Sequence[int], role: str = "train", seed: int = 0):
        ...
        if group_ratios and (role_map[self.role] != 2):
            ...
            print(f"No Eyeglasses: {label_counts[0]}, Eyeglasses: {label_counts[1]}")
            ...
            print(f"No Eyeglasses: {label_counts_after[0]}, Eyeglasses: {label_counts_after[1]}")
            df = df[idx]

        self.filename = df["image_id"].tolist()
        # Wearing glasses is 1, otherwise zero
        self.y = torch.Tensor(df["Eyeglasses"].values).long()
        # Male is 1, Female is 0
        self.z = torch.Tensor(df["Male"].values).long()
